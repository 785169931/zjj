...
Route::group(['namespace'=>'Admin','prefix'=>'admin','middleware'=>['auth','permission:system.manage']],function (){
    ...
    //测试数据
    Route::group(['middleware' => 'permission:apply.test'],function (){
       Route::get('apply/data','ApplyController@data')->name('apply.test');
    });
});
...
//供应链管理
Route::group(['namespace' => 'Admin','prefix' => 'admin','middlewear' => ['auth','permission:apply.manager']],function (){
    Route::group(['middlewear' => 'permission:apply.manager'],function (){
        Route::get('apply/data','ApplyController@data')->name('admin.apply.data');

        Route::get('apply','ApplyController@index')->name('admin.apply');

        Route::get('apply/create','ApplyController@create')->name('admin.apply.create')->middleware('permission:apply.apply.create');
        Route::post('apply/store','ApplyController@store')->name('admin.apply.store')->middleware('permission:apply.apply.create');

        Route::get('apply/download','ApplyController@download')->name('admin.apply.download')->middleware('permission:apply.apply.download');

        Route::any('apply/destory','ApplyController@destory')->name('admin.apply.destory')->middleware('permission:apply.apply.destory');
    });
});

Route::namespace('Admin')->group(function (){
    Route::post('post','CommonController@post')->name('admin.post');
});
